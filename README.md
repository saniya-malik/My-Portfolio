
A modern personal portfolio showcasing projects, skills, and experience.
# Portfolio Website

A modern, dark-themed personal portfolio built using **HTML, CSS, and JavaScript**.  
This website highlights my projects, skills, and experience with a clean and professional UI.

---

## Features
- Smooth dark theme with soft gradients  
- Modern, minimal, and responsive design  
- Interactive hero section  
- Projects showcased with clean UI  
- Footer with social icons (GitHub, LinkedIn, Mail)  
- Custom CSS animations and subtle effects  
- Fully responsive across devices  

---

## Tech Stack
- **HTML**
- **CSS**
- **JavaScript**

---

## 📂 Project Structure
